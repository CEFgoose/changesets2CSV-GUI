...
def create_summary(args,input_file,export_dir):
    changeset_total = 0
    edit_total = 0
    disc_total = 0
    # edits per changeset
    ...
    with open(input_file, mode="r") as f:
        ...
        elif input_file.endswith(".config"):
        ...
        for user in users:
            ...
            out_file = os.path.join(export_dir, str(user["name"]) + ".csv")
...
def summary_csv(args, output_file, json_dict, summary):
    basepath = os.getcwd()
    summary_file=basepath+output_file
...
def changeset_csv(output_file, json_dict, changesets, name=None, summary=False):
    try:
        basepath=os.getcwd()
        output_file=  basepath+output_file
...
def get_args(method,input_file,export_dir):
    ...
    weekly.add_argument("input_file", default=input_file,help="Path of the .json file of users and tags")
